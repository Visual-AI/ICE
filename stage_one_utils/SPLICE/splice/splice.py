...
def load(name: str, vocabulary: str, vocabulary_size: int = 10000, device = "cuda:0" if torch.cuda.is_available() else "cpu", download_root = None, **kwargs):
    """load SpLiCE
    ...
    if vocabulary in SUPPORTED_VOCAB:
        ...
        if os.path.isfile(concept_path):
            concepts = torch.load(concept_path, map_location=device)
        else:
